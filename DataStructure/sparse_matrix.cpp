...
Matrix createMatrix(int row_n, int col_n, int n)
{
	...
	int row, col, value;
	M->num = n;
	M->col_num = col_n; M->row_num = row_n;//初始化各参数
...
Matrix multiply(Matrix a, Matrix b)
{
	Matrix c = createMatrix(0, 0, 0);
	int i, j;
	int r_index = 1, c_index = 1;//r_index,c_index分别记录结果矩阵中元素行标、列标
	int a_index = 0;//记录矩阵A三元组表中元素下标
	while (a_index < a->num)
	{
		if (a->data[a_index].row > r_index)r_index++;//比较行标
		else if (a->data[a_index].row < r_index)a_index++;
		else//行标相同
		{
